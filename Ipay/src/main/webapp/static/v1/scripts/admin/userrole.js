...
function delUser(id){
	...
	if(window.confirm('确定要删除？')){
		$.ajax({
			type : "POST",
			url : "wx/admin/authuser/deluser.json",
			data : {
				oid : id
			},
			datatype : "json",
...
function setUserValue(data) {
	$addUserTitle.text("修改用户信息--" + data.name);
	$('#OID').val(data.oID);
...
function saveUserRole() {
	...
	$('input[name="roles"]:checked').each(function() {
		json.roleName.push($(this).val());// 将选中的值添加到数组roles中
		json.userOID.push(oid);// 将选中的值添加到数组roles中
	});
