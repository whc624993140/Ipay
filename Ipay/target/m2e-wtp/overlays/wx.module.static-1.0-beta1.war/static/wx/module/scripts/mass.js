$(function(){
	...
	$("#-999").click(function(){
		...
		$("#mass_label_selected").append( "<div class='tagName fl ' key='all' >"+wx_lang.page_key273+"<span name='del_label_selected'>X</span></div>");
		updatePersonTotal();
	});
...
function updatePersonTotal(){
	...
	for (var int = 0; int < labels.length; int++) {
		if(labels[int]=='all'){
			$("#totalPerson").html(wx_lang.page_key273);
			return;
		}
	}
	$.ajax({
...
var masssend={
	text : function(label,content) {
		var params={content:content,labels:label};
		var initUrl = "wx/admin/mass/labels/text/send.json";
		if(label=='all'){
			initUrl = "wx/admin/mass/all/text/send.json";
		}
		$.ajax({
			type : 'post',
			traditional : true,
		...
		var params={newsid:newsid,labels:label};
		var initUrl = "wx/admin/mass/labels/news/send.json";
		if(label=='all'){
			initUrl = "wx/admin/mass/all/news/send.json";
		}
		$.ajax({
			type : 'post',
			traditional : true,
		...
		var params={src:src,labels:label};
		var initUrl = "wx/admin/mass/labels/image/send.json";
		if(label=='all'){
			initUrl = "wx/admin/mass/all/image/send.json";
		}
		$.ajax({
			type : 'post',
			traditional : true,
		...
		var params={src:src,labels:label};
		var initUrl = "wx/admin/mass/labels/voice/send.json";
		if(label=='all'){
			initUrl = "wx/admin/mass/all/voice/send.json";
		}
		$.ajax({
			type : 'post',
			traditional : true,
