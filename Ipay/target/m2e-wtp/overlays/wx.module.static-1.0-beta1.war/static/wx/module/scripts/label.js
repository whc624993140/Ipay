$(function() {
	...
	$('.addlabels').click(function() {
		...
		if (_parent.attr('id') == 'labelFirst') {
			$('.labelMask_t span').text(wx_lang.page_key326);
			$('#level').val('1');
			$('#parentid').val($("#parentid_1").val());
		} else if (_parent.attr('id') == 'labelSecond') {
			$('.labelMask_t span').text(wx_lang.page_key327);
			$('#level').val('2');
			$('#parentid').val($("#parentid_2").val());
		} else if (_parent.attr('id') == 'labelThird') {
			$('.labelMask_t span').text(wx_lang.page_key328);
			$('#level').val('3');
	...
	$(document).on('click', '.label_ed',function(e) {
		...
		$('.labelMask_t span').text(wx_lang.page_key98+wx_lang.page_key330+'#' + data.name);
		$('.labelMask').show();
	});
